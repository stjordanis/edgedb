...
Define a new property ``number_of_connections`` as a computable on the
``User`` object type counting the number of interests:
...
:eql:synopsis:`USING (<computable-expr>)`
    Change the expression of a computable property.  Only valid for
    concrete properties.
...
Redefine the computable property ``number_of_connections`` to be the
number of friends:

...
Define a new link ``special_group`` as a computable on the ``User``
...
:eql:synopsis:`USING (<computable-expr>)`
    Change the expression of a computable link.  Only valid for
    concrete links.
...
Redefine the computable link ``special_group`` to be those who have
